...
public class ActiveDocumentsEndpoint
		extends
...
//	Вещь, создающая контент для столбца "Краткое содержание"
	...
	public ActiveDocumentsEndpoint(Marshaller marshaller) {
		super(marshaller);
	}
...
DocumentsTableRequest request = requestDocument.getDocumentsTableRequest();
		...
		List<Long> ids = facade.getActiveDocumentIDs(filter);
		List<ETDDocument> documents = facade.getActiveDocuments(filter, ids);
...
//						System.out.println("if");
		...
		if(sort!=null && (sort.equalsIgnoreCase("short") /*|| sort.equalsIgnoreCase("number")*/)){
			...
			/*else if(sort.equalsIgnoreCase("number"))
			    {System.out.println("number");
			    dc = sorter.sortByDocumentNumber(dc, sortDir);
			    }*/
