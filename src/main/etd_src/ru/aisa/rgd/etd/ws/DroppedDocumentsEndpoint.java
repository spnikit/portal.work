...
public class DroppedDocumentsEndpoint
		extends
...
//	Вещь, создающая контент для столбца "Краткое содержание"
	...
	public DroppedDocumentsEndpoint(Marshaller marshaller) {
		super(marshaller);
	}
...
DocumentsTableRequest request = requestDocument.getDocumentsTableRequest();
		...
		List<Long> ids = facade.getDroppedDocumentIDs(filter);
		List<ETDDocument> documents = facade.getDroppedDocuments(filter, ids);
