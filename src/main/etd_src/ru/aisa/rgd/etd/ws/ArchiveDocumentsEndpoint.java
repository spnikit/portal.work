...
public class ArchiveDocumentsEndpoint extends ETDAbstractSecurityEndoint<DocumentsTableRequestDocument, DocumentsTableResponseDocument> {
...
//	Вещь, создающая контент для столбца "Краткое содержание"
	...
	public ArchiveDocumentsEndpoint(Marshaller marshaller) {
		super(marshaller);
	}
...
DocumentsTableRequest request = requestDocument.getDocumentsTableRequest();
		...
		List<Long> ids = facade.getArchiveDocumentIDs(filter);
		List<ETDDocument> documents = facade.getArchiveDocuments(filter, ids);
...
//			xml.setShort(shortContentCreator.createValue(doc.getName(), doc.getData()));
		...
		if(sort!=null && (sort.equalsIgnoreCase("short"))){
