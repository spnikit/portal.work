...
public class SignPackCSS extends AbstractPortalSimpleController {

	
	private boolean drop = false;
	
	
	public SignPackCSS() throws JSONException {
		super();
	}
		...
		getNpjt().query(sql,pp, new ParameterizedRowMapper<Object>() {
			...
			public Object mapRow(ResultSet rs, int numrow) throws SQLException {
				...
				if (!TORLists.autosign.contains(rs.getString("name"))){
					if (rs.getInt("drop")==1)
						drop = true;
					else if (TORLists.acceptlist.contains(rs.getString("name"))
							&&rs.getInt("visible")!=2){
