package sheff.rjd.ws.OCO.AfterSign.TOR;
...
public class NumberOfWords {
	 ...
	 * 
	 * @param stripkop
	 *            boolean флаг - показывать копейки или нет
	...
	public String num2str(boolean stripkop) {
		String[][] sex = {
				...
				{ "", "одна", "две", "три", "четыре", "пять", "шесть", "семь",
						"восемь", "девять" }, };
		String[] str100 = { "", "сто", "двести", "триста", "четыреста",
				"пятьсот", "шестьсот", "семьсот", "восемьсот", "девятьсот" };
		String[] str11 = { "", "десять", "одиннадцать", "двенадцать",
				"тринадцать", "четырнадцать", "пятнадцать", "шестнадцать",
				"семнадцать", "восемнадцать", "девятнадцать", "двадцать" };
		String[] str10 = { "", "десять", "двадцать", "тридцать", "сорок",
				"пятьдесят", "шестьдесят", "семьдесят", "восемьдесят",
				"девяносто" };
		String[][] forms = { { "копейка", "копейки", "копеек", "1" },
		...
		if (rub == 0) {// если Ноль
			...
			if (stripkop)
				...
				return o + " " + kop + " "
						+ morph(kop, forms[0][0], forms[0][1], forms[0][2]);
		}
		// Больше нуля
		int lev = segments.size();
		for (int i = 0; i < segments.size(); i++) {// перебираем сегменты
			int sexi = (int) Integer.valueOf(forms[lev][3].toString());// определяем
																		// род
			int ri = (int) Integer.valueOf(segments.get(i).toString());// текущий
																		// сегмент
			if (ri == 0 && lev > 1) {// если сегмент ==0 И не последний
										// уровень(там Units)
				lev--;
				continue;
			}
			...
			int r22 = (int) Integer.valueOf(rs.substring(1, 3)); // вторая и
																	// третья
