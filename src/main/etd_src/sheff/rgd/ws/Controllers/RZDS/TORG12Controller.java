	...
	public void doAfterSign(String docName, String docdata, int predid,
		...
		if (signNumber == 1){
			try {
				...
				int newpred= npjt.queryForInt("select id from snt.pred where okpo_kod =:okpo", hm1);
				String id_pak = (String) npjt.queryForObject("select id_pak from snt.docstore where id =:id", hm1, String.class);
				hm1.put("id_pak", id_pak);
				hm1.put("newpred", newpred);
				npjt.update("update snt.docstore set predid =:newpred where id_pak =:id_pak", hm1);
		...
		if (signNumber == 2 || drop == 1) {
			...
			try {
				...
				List<Map<String, Object>> ids = getNpjt()
						.queryForList(
