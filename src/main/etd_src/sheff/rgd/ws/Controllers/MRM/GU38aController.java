...
public class GU38aController extends AbstractAction{
		...
		if (obj.getSignlvl()==0){
			try{
				 StringBuffer no = new StringBuffer();
				  ...
				  content.append(", ");
				  content.append(b.getValue("P_6_3"));
				  content.append(", ");
				  content.append(b.getValue("P_6_4"));
				  content.append(", ");
				  content.append(b.getValue("P_6_6"));
...
//				e.printStackTrace();
	...
	if (obj.getSignlvl()>1){
		...
		npjt.update("update snt.docstore set docdata =:docdata, bldoc = :bldoc, signlvl =:signlvl where etdid =:etdid", pp);
			...
			try {
				if (b.getValue("P_6_1").equals("000"))
				{
				...
				else if (!b.getValue("P_6_1").equals("000")){
					if (b.getValue("P_28").length()>2&&b.getValue("P_31").length()>2&&b.getValue("P_34_3").length()>2)
						npjt.update("update snt.docstore set signlvl = null where etdid =:etdid", pp);
				}
			} catch (Exception e) {
				log.error(TypeConverter.exceptionToString(e));
