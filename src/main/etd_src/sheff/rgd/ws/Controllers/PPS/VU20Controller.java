...
public class VU20Controller extends AbstractAction{
	...
	public void doAfterSign(String docName, String docdata, int predid, int signNumber, long id, String certserial, int WrkId) {
		try {
			if(signNumber == 1) {
				...
				binder.setNodeValue("akt_date", sdf.format(dateSign));
				...
				ETDForm form = ETDForm.createFromArchive(docdata.getBytes("UTF-8"));
				DataBinder binder = form.getBinder();
				Long idClaim = Long.parseLong(binder.getValue("P_16"));
				updateClaim(idClaim, binder);
				Map<String, Object> map = new HashMap<String, Object>();
				map.put("id", id);
				SqlRowSet rs = getNpjt().queryForRowSet("select id, type_act, type_act_1,"
						+ " id_app_treatment, id_app_treatment_1, case WHEN id_act_workability is not null THEN 1 end p1,"
				...
				while(rs.next()) {
					...
					map.put("docid", rs.getLong("id"));
					map.put("name_prod_from", binder.getValue("productIn"));
					map.put("name_prod_to", binder.getValue("productOut"));
					map.put("num_operation", binder.getValue("oper"));
					map.put("num_act", binder.getValue("vu_number"));
					SimpleDateFormat sdf = new SimpleDateFormat("dd.MM.yyyy");;
					map.put("date_act", sdf.parse(binder.getValue("akt_date")));
					map.put("reason", binder.getValue("P_25"));
					if(rs.getInt("type_act") != 0 && rs.getInt("type_act") != 3) {
						if(rs.getObject("id_app_treatment_1") != null) {
							id_claim = rs.getLong("id_app_treatment_1");
						}
						getNpjt().update("update snt.ppsreport set name_prod_from = :name_prod_from, "
								...
								+ "num_act = :num_act, date_act = :date_act, reason = :reason "
								+ "where id = :docid ", map);
					}
					if(rs.getInt("type_act_1") != 0 && rs.getInt("type_act_1") != 3) {
						id_claim = rs.getLong("id_app_treatment");
						getNpjt().update("update snt.ppsreport set name_prod_from_1 = :name_prod_from, "
								...
								+ "num_act_1 = :num_act, date_act_1 = :date_act, reason = :reason "
								+ "where id = :docid", map);
					}
					
				}
				if(id_claim != null) {
					updateClaim(id_claim, binder);
				}
	...
	public void doAfterSave(String docName, String docdata, int predid, int signNumber, String CertID, long id, int WrkId) throws Exception {
		...
		while(rs.next()) {
			...
			if(rs.getInt("type_act") != 0 && rs.getInt("type_act") != 3) {
				getNpjt().update("update snt.ppsreport set name_prod_from = :name_prod_from, "
						+ "name_prod_to = :name_prod_to, num_operation = :num_operation, "
						+ " reason = :reason where id = :docid ", map);
			}
			if(rs.getInt("type_act_1") != 0 && rs.getInt("type_act_1") != 3) {
				getNpjt().update("update snt.ppsreport set name_prod_from_1 = :name_prod_from, "
						+ "name_prod_to_1 = :name_prod_to, num_operation_1 = :num_operation, "
		...
		String content = binder.getValue("cist") + " " + binder.getValue("productOut");
		map.put("opisanie", content);
...
//			e.printStackTrace();
	...
	private void updateClaim(Long idClaim, DataBinder binder) throws UnsupportedEncodingException, ServiceException, IOException, InternalException, TransformerException {
		Map<String, Object> map = new HashMap<String, Object>();
		...
		final String vagnum = binder.getValue("cist");
		final String P_14InClaim = "ВУ-20 " + binder.getValue("vu_number") + " " + binder.getValue("akt_date");
