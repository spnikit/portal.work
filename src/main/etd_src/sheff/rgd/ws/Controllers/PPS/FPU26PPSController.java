...
public class FPU26PPSController extends AbstractAction{
	...
	public void doAfterSign(String docName, String docdata, int predid, int signNumber, long id, String certserial, int WrkId) {
		...
		reestr = facade.getDocumentDao().getByEtdId(etdreestrid);
		reestrid = etdsyncfacade.getIdbyEtdid(etdreestrid);
		}catch (Exception e){
		...
		if (signNumber==2){
			...
			if (reestrid>-1){
			formControllers.doAfterSave(reestr.getType(), new String(reestr.getBlDoc(), "UTF-8"), predid, signNumber, certserial, reestrid, WrkId);
			}
			} catch (Exception e) {
				log.error(TypeConverter.exceptionToString(e));
			}
		...
		try{
		 ...
		 if (readid==-1){
			 ...
			 form = ETDForm.createFromArchive(docdata.getBytes("UTF-8"));
				DataBinder kinder = form.getBinder();
			long etdreestrid = Long.parseLong(kinder.getValue("P_46"));
			hm1.put("etdreestrid", etdreestrid);
			 hm1.put("reqnum", "Согласовано");
			 npjt.update("update snt.docstore set readid = 1, reqnum =:reqnum where id =:ID", hm1);
			 npjt.update("update snt.docstore set reqnum =:reqnum where etdid =:etdreestrid", hm1);
		 }
		} catch (Exception e){
			log.error(TypeConverter.exceptionToString(e));
		    }
