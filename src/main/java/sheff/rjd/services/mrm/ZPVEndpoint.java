...
import ru.iit.zpv.zpvEndPoint.FormCreateRequestDocument;
import ru.iit.zpv.zpvEndPoint.FormCreateRequest;
import ru.iit.zpv.zpvEndPoint.FormCreateRequest.Table1.Row;
...
import ru.iit.zpv.zpvEndPoint.FormCreateResponse;
import ru.iit.zpv.zpvEndPoint.FormCreateResponseDocument;


public class ZPVEndpoint extends ETDAbstractEndpoint<StandartResponseWrapper>{
    
	private NamedParameterJdbcTemplate npjt;  
	private DataSource ds;
	private String formname;
	protected ZPVEndpoint(Marshaller marshaller) {
		super(marshaller); 
	}
	...
	protected ResponseAdapter<StandartResponseWrapper> processRequest(Object arg, ServiceFacade facade)
		...
		if (reWriteDoc) {
			...
			//binder.clearElements("P_1b", "P_1v","P_7_1","P_7_2","P_7_3","P_7_4","P_8_1","P_8_2","P_14a","P_14b","P_14v");
			//binder.clearTable("table1", "row");
        ...
        if(req.isSetPDateStartWork()){
        binder.setNodeValue("P_7_1", req.getPDateStartWork());
        }
        if(req.isSetPTimeStartWork()){
        binder.setNodeValue("P_7_2", req.getPTimeStartWork());
        }
        binder.setNodeValue("P_7_3", req.getPDateStartWork());
        binder.setNodeValue("P_7_4", req.getPTimeStartWork());
		if(req.isSetDateEnd()){
		...
		try{
			...
			content.append(", ");
			if(req.isSetPDateStartWork()){
				content.append(req.getPDateStartWork());
				content.append(", ");
			}
			if(req.isSetPTimeStartWork()){
				content.append(req.getPTimeStartWork());
				content.append(", ");
			}
			if(req.isSetDateEnd()){
