...
import ru.iit.rzds.sfs.AddrIn;
import ru.iit.rzds.sfs.AddrRf;
import ru.iit.rzds.sfs.SFRequestDocument;
import ru.iit.rzds.sfs.SFRequestDocument.SFRequest;
import ru.iit.rzds.sfs.Table1;
import ru.iit.rzds.sfs.Table2;

import com.aisa.portal.invoice.integration.security.Manager;
import com.aisa.portal.invoice.operator.obj.OperatorObject;


public class SF_Service_Endpoint extends ETDAbstractEndpoint<StandartResponseWrapper> {

	private static Logger log = Logger.getLogger(SF_Service_Endpoint.class);
	...
	public SF_Service_Endpoint(Marshaller marshaller) {
		super(marshaller);
	}
...
//			binder.setNodeValue("id_pak", request.getIdPak());
			...
			if (request.isSetGo())
				binder.setNodeValue("P_47", request.getGo());
			if (request.isSetGp())
				binder.setNodeValue("P_48", request.getGp());
...
//								float C = Float.parseFloat(row.getNalSumm());
			...
			binder.setNodeValue("P_45", request.getDateservice());
			binder.setNodeValue("P_42a", request.getActnumber());
			binder.setNodeValue("P_42b", request.getActdate());
			binder.setNodeValue("P_46a", request.getActsfnumber());
			binder.setNodeValue("P_46b", request.getActsfdate());
			...
			pp.put("okpo_code", request.getOkpoCodeSell());
			ent1 = facade.getNpjt().queryForInt("select id from snt.pred where okpo_kod =:okpo_code", pp);
