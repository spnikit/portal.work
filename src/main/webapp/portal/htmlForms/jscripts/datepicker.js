...
(function($) {
	...
	var DatePicker = function() {
		...
		var ids = {}, views = {
			...
			wrapper : '<div class="datepicker" style="z-index: 15;"><div class="datepickerBorderT" /><div class="datepickerBorderB" /><div class="datepickerBorderL" /><div class="datepickerBorderR" /><div class="datepickerBorderTL" /><div class="datepickerBorderTR" /><div class="datepickerBorderBL" /><div class="datepickerBorderBR" /><div class="datepickerContainer"><table cellspacing="0" cellpadding="0"><tbody><tr></tr></tbody></table></div></div>',
			head : [
					'<td>',
			...
			locale : {
				...
				months : ["Январь", "Февраль", "Март", "Апрель", "Май", "Июнь",
						"Июль", "Август", "Сентярбь", "Октябрь", "Ноябрь",
						"Декабрь"],
			...
			if (el.is('a')) {
				...
				if (parentEl.is('th')) {
					...
					switch (tblEl.get(0).className) {
						case 'datepickerViewMonths' :
							options.current.setMonth(tblEl
									.find('tbody.datepickerMonths td')
									.index(parentEl));
							options.current.setFullYear(parseInt(
									tblEl.find('thead th.datepickerMonth span')
											.text(), 10));
						...
						case 'datepickerViewYears' :
							options.current
									.setFullYear(parseInt(el.text(), 10));
							tblEl.get(0).className = 'datepickerViewMonths';
							break;
						default :
							...
							switch (options.mode) {
								...
								case 'range' :
									if (!options.lastSel) {
										options.date[0] = (tmp.setHours(0, 0,
												0, 0)).valueOf();
									}
									val = (tmp.setHours(23, 59, 59, 0))
											.valueOf();
		...
		var retVar2 = {
			init : function(options) {
				...
				options.calendars = Math.max(1, parseInt(options.calendars, 10)
								|| 1);
					...
					if (!$(this).data('datepicker')) {
						...
						for (var i = 0; i < options.calendars; i++) {
							...
							html += tmpl(tpl.head.join(''), {
										...
										prev : options.prev,
										next : options.next,
										day1 : options.locale.daysMin[(cnt++)
												% 7],
										day2 : options.locale.daysMin[(cnt++)
												% 7],
										day3 : options.locale.daysMin[(cnt++)
												% 7],
										day4 : options.locale.daysMin[(cnt++)
												% 7],
										day5 : options.locale.daysMin[(cnt++)
												% 7],
										day6 : options.locale.daysMin[(cnt++)
												% 7],
										day7 : options.locale.daysMin[(cnt++)
												% 7]
									});
						}
						cal.find('tr:first').append(html).find('table')
			...
			setDate : function(date, shiftTo) {
				...
				return this.each(function() {
					...
					if ($(this).data('datepickerId')) {
						...
						if (shiftTo) {
							options.current = new Date(options.mode != 'single'
											? options.date[0]
											: options.date);
						}
						fill(cal.get(0));
