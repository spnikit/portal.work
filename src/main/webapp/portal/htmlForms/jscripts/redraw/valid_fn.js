$(document).ready(function(){
	...
	//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
	//вешаем на каждый элемент с описанием format,подстановка параметров и выбор шаблона подставляется при генерации
	//привязывается по id
	$("#text_for_valid_3").blur(function() {
		//====================================================================================
		//входные данные (format):
		var datatype='integer';//тип вводимых данных, определяет дефолтный шаблон
		//costrains:
		var check='none';//проверять ли
		var message="get it from 'message'";//тест для вывода во всплывающей подсказке
		var mandatory='on';//обязательный ли элемент
		var length={min:0,max:32768};//длина вводимого значения в символах [32kB for string. numb +-2^64=>64] 
		var patterns={};//регулярные выражения, которым должно соответствовать
		var range={min:'',max:''};//min и max для введенного значения
		var template='';//шаблон для ввода. для телефона: (...)...-..-..
		//presentation //набор для разных типов данных различен, пока нереализовано. 
		//====================================================================================
		...
		//для связанных с датами использубтся различные формы вывода medium,full и т.д.
		//нет проверок на корректность введенного времени[41.49.9999 51:99]
		if(datatype=='string')
		{
			match='^.{'+length.min+','+length.max+'}$';
			length={min:minV,max:maxV};
		}
		...
		{
			match='^[0-9]{'+length.min+','+length.max+'}\\.[0-9]{'+length.min+','+length.max+'}$';
			length={min:minV,max:maxV};
		}
		...
		{
			match='^[0-9]{'+length.min+','+length.max+'}\.[0-9]{2,2}$';
			length={min:minV,max:maxV};
		}
		...
		if($(this)[0].value.length!=0 && ( 
				...
				|| $(this)[0].value.length < length.min 
				|| $(this)[0].value.length > length.max ))
		{
...
jQuery.fn.simpeValidate = function(options) {		
	var options = jQuery.extend({
		//====================================================================================
		//входные данные (format):
		datatype:'string',	//тип вводимых данных, определяет дефолтный шаблон
		//costrains:
		check:'none',	//проверять ли
		message:'',		//тест для вывода во всплывающей подсказке
		mandatory:'off',	//обязательный ли элемент
		length:{min:0,max:32768},//длина вводимого значения в символах [32kB for string. numb +-2^64=>64] 
		patterns:{},//регулярные выражения, которым должно соответствовать
		range:{min:'',max:''},//min и max для введенного значения
	...
	{
		match='^[0-9]{'+options.length.min+','+options.length.max+'}\.[0-9]{2,2}$';
	}
	...
	$(this).blur(function(){
		if($(this)[0].value.length!=0 && ( 
				...
				|| $(this)[0].value.length < options.length.min 
				|| $(this)[0].value.length > options.length.max ))
		{
			$(this).addClass('invalid_input').tooltip({ 
				txt: options.message
			});
