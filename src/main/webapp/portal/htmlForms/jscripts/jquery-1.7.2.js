		...
		for ( var i = 0, elem; (elem = elems[i]) != null; i++ ) {
			...
			if ( typeof elem === "string" ) {
				if ( !rhtml.test( elem ) ) {
					...
					// Clear elements from DocumentFragment (safeFragment or
					// otherwise)
		...
		function doAnimation() {
			...
			for ( p in prop ) {
				...
				if ( ( hooks = jQuery.cssHooks[ name ] ) && "expand" in hooks ) {
					...
					// present.
					// also - reusing 'p' from above because we have the correct
					// "name"
					for ( p in replace ) {
						if ( ! ( p in prop ) ) {
							prop[ p ] = replace[ p ];
	...
	jQuery.fn[ type ] = function( value ) {
		return jQuery.access( this, function( elem, type, value ) {
			...
			if ( elem.nodeType === 9 ) {
				...
				// when a window > document, IE6 reports a offset[Width/Height]
				// > client[Width/Height]
				// so we can't use max, as it'll choose the incorrect
				// offset[Width/Height]
