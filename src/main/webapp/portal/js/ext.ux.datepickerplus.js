...
//calculate the HTML of one month at first to gain some speed when rendering many calendars
		...
		m.push('<tr><td',(this.noOfMonthPerRow>1?' colspan="'+this.noOfMonthPerRow+'"':''),' class="x-date-bottom x-aisa-date" align="center"><div><table width="100%" cellpadding="0" cellspacing="0">',
				'<tr><td align="right" class="x-date-multimonthbtn">'/*,widfaker,'<td align="center" class="x-date-nullbtn">',widfaker*/,'<td align="left" class="x-date-multiallbtn"></tr>',
				'<tr><td align="right" class="x-date-multiokbtn">'/*,widfaker,'</td><td align="center" class="x-date-todaybtn">',widfaker*/,'</td><td align="left" class="x-date-multiundobtn">',widfaker,'</td></tr></table></div></td></tr>');
...
//showtoday from Ext 2.2
		...
		this.allBtn = new Ext.Button({
	            renderTo: this.el.child("td.x-date-bottom .x-date-multiallbtn", true),
				text: '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Все&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;',
				//autoWidth:false,
				/*handler: function() {
					...
					//this.update(today);
					//this.markDateAsSelected(new Date('01.01.2000').getTime(),false,this.todayMonthCell,this.todayDayCell,true);
					//this.markDateAsSelected(new Date('01.01.2000').getTime(),false,4,13,true);
					
				},*/
				handler: this.allClicked,
				scope: this
			});
		this.MnthBtn = new Ext.Button({
            renderTo: this.el.child("td.x-date-bottom .x-date-multimonthbtn", true),
			text: 'За месяц',
			tooltip:'отображение документов за весь месяц, выбранный в календаре',
			//boxMinWidth:100,
			/*handler: function() {
				this.clearSelectedDates();
			...
			handler: this.monthClicked,
			scope: this,
			listeners:{'render':{fn:function(button){button.el.style='width:100%;';},scope:this}}
		});
...
if(Ext.ux.DatePickerPlus){
	Ext.apply(Ext.ux.DatePickerPlus.prototype, {
		...
		maxSelectionDaysText: 'Вы можете выбрать не больше 31-го дня',
		undoText: "Отменить",
