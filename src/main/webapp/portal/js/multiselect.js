...
Ext.form.MultiSelectField = Ext.extend(Ext.form.TriggerField, {
    ...
    listeners: {
    	blur : function(f) {
            this.setValue('',f.hiddenSuperval);
        }
    },
 ...
 vovako: function() {
    ...
    onTriggerClick: function() {
      ...
      //  var width = this.containerWidth != '' ? this.containerWidth : (
       ...
       // this.menu.el.dom.style.overflowX = 'hidden';
        this.setValue('',this.hiddenSuperval);
        this.populateList(this.value);
    },
    ...
    setValue: function(value, text) {
        ...
        if (text == undefined && value != undefined) {
      	 ...
      	 var valueOut="";
    	 var textOut="";
            var sel = text.split(this.textSeparator);
            for (i = 0; i < sel.length; i++) {
                try {
                    var value1 = this.store.query(this.displayField, 	new RegExp("^"+ sel[i]+"$")).items[0].data[this.valueField];
                    this.menu.items.each(function(mi) {
                	   
                       if (mi.value == value1) 
                       {
                       valueOut += (valueOut != '' ? this.valueSeparator : '') + value1;
                       textOut += (textOut != '' ? this.textSeparator : '') +sel[i];
                       } else {}  
                    }, this);
                } catch (e) { }
              }
...
//        	 alert(keys[i]);
            if ((keys[i] != undefined && keys[i] != '')||keys[i] =='0' ) {
                // get the full store object
            	var reg = new RegExp("^"+ keys[i]+"$")
                var item = this.store.query(this.valueField, reg).items[0];
                if (item != undefined) {
    ...
    populateList: function(v) {
        ...
        if (v != undefined && v != '' && v != null) {
        	...
        	for (i = 0; i < sel.length; i++) {
        		try {
        			var value = sel[i];//this.store.query(this.valueField, sel[i]).items[0].data[this.valueField];
        			this.menu.items.each(function(mi) {
